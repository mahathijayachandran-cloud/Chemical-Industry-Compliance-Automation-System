...
interface OSHAComponentProps {
  chemicals: string[];
}

export default function OSHAComponent({ chemicals }: OSHAComponentProps) {
  const [chemical, setChemical] = useState("");
  const [exposureValue, setExposureValue] = useState<number | "">("");
  const [result, setResult] = useState<ComplianceResult | null>(null);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);

  const handleCheck = async () => {
    if (!chemical || exposureValue === "") return;
    setLoading(true);
    setError(null);
    try {
      const response = await fetch("/api/check/osha", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ chemical, exposureValue }),
      });
    ...
    <div className="space-y-6">
      <div className="bg-zinc-900 border border-zinc-800 p-6 rounded-xl shadow-2xl">
        <h2 className="text-xl font-bold text-white mb-6 flex items-center gap-2">
          ...
          OSHA Exposure Compliance Checker
        </h2>
        ...
        <button
          onClick={handleCheck}
          disabled={loading || !chemical || exposureValue === ""}
        ...
        >
          {loading ? <Loader2 className="animate-spin" /> : "Check OSHA Compliance"}
        </button>
        ...
        <div className={cn(
          ...
          <div className="flex items-start justify-between mb-4">
            <div>
              <div className={cn(
                ...
                {result.isViolation ? "PEL Violation" : "Safe Exposure Level"}
              </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
            <div className="bg-zinc-900/50 p-3 rounded-lg border border-zinc-800">
              <p className="text-zinc-500 text-xs uppercase font-bold">Measured</p>
              <p className="text-xl font-mono text-white">{result.exposureValue} ppm</p>
            </div>
            <div className="bg-zinc-900/50 p-3 rounded-lg border border-zinc-800">
              <p className="text-zinc-500 text-xs uppercase font-bold">PEL Limit</p>
              <p className="text-xl font-mono text-white">{result.limit} ppm</p>
            </div>
            <div className="bg-zinc-900/50 p-3 rounded-lg border border-zinc-800">
              ...
              <p className={cn(
                ...
                {((result.exposureValue || 0) - result.limit).toFixed(2)} ppm
              </p>
            </div>
          </div>
          ...
          <div className="space-y-4">
            <div>
              <h4 className="text-sm font-bold text-zinc-400 uppercase tracking-widest mb-1">Industrial Hygiene Analysis</h4>
            ...
            <div>
              <h4 className="text-sm font-bold text-zinc-400 uppercase tracking-widest mb-1">Required Controls</h4>
              <div className={cn(
